...
bool DSPVLIWBundlerDriver::runOnMachineBasicBlock(MachineBasicBlock &MBB){
	...
	{
		if (MI->isBundle()){
			std::vector<MachineInstr *> package;
			MachineBasicBlock::instr_iterator MII = MI;
			++MII;
			unsigned int IgnoreCount = 0;
			while (MII != MIE && MII->isInsideBundle()){//MIλ��ͬһ��Bundle��push��package��
			...
			for (unsigned int i = 0; i < size; i++)
			{
				...
				unsigned int slot =CurrentSlot.SlotMask;
				unsigned int mask = CurrentSlot.mask;
			...
			for (unsigned int i = 0; i < size; i++)//ppp
				for (unsigned j = 0; j < size - i - 1; j++)
					if (Bund->InstrToSlot[package[j]] > Bund->InstrToSlot[package[j+1]]) {
						MachineInstr * tmp = package[j + 1];
						package[j + 1] = package[j];
						package[j] = tmp;
					}
			for(int i=0;i<size;i++){
				auto ins = MBB.remove_instr(package[i]);
				MBB.insert(MII,ins);
				ins->bundleWithPred();
			}
		}
